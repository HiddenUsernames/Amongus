...
function Multiplayer.JoinLobby(HostName)
    ...
    print("Spamming AcceptInvite infinitely until successfully inside the party UI...")

    -- Infinite loop: will keep trying to accept until the UI confirms we are in slot '1'
    while true do
        local success, joined = pcall(function()
            local reactLobby = playerGui:FindFirstChild("ReactLobbyParty")
            local party = reactLobby and reactLobby:FindFirstChild("party")
            local currentParty = party and party:FindFirstChild("currentParty")
            local partyMembers = currentParty and currentParty:FindFirstChild("partyMembers")
            
            -- If the slot '1' exists, we have successfully joined the host's party
            if partyMembers and partyMembers:FindFirstChild("1") then
                return true
            end
            return false
        end)

        -- Break the loop if the pcall succeeded and confirmed we are in the lobby
        if success and joined then
            break
        end

        -- Keep firing the remote event asynchronously so it doesn't throttle the loop
        coroutine.wrap(function()
            Event:InvokeServer("Party", "AcceptInvite", hostPlayer)
        end)()

        -- Fast interval to ensure rapid firing until connection registers
        task.wait(0.2)
    end

    print("Successfully joined " .. hostPlayer.Name .. "'s lobby and confirmed by UI!")
end
