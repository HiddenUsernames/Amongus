...
function TDS:Addons()
    ...
    -- Define a helper function to handle a single download/execution attempt
    ...
    -- Keep retrying until it successfully loads
    while not loadSuccess do
        -- Fire off the download attempt asynchronously so we can monitor the time
        local attemptFinished = false
        task.spawn(function()
            attemptSuccess = attemptLoad()
            attemptFinished = true
        end)

        -- Monitor the 15-second timeout
        local startTime = os.clock()
        while not attemptFinished and (os.clock() - startTime) < 15 do
            task.wait(0.1)
        end

        -- If it finished successfully AND the place changed (per your original logic), we break out
        if attemptFinished and attemptSuccess then
            -- Optional: If your script relies on self.Place changing to verify success, 
            -- we wait up to the remainder of the time or just verify it here.
            local placeWaitStart = os.clock()
            while self.Place == originalPlace and (os.clock() - placeWaitStart) < 5 do
                task.wait(0.1)
            end
            
            if self.Place ~= originalPlace then
                loadSuccess = true
            end
        end

        if not loadSuccess then
            warn("[TDS] Load timed out or failed. Retrying script refresh...")
        end
    end
...
local function DoPlaceTower(TName, TPos, ...)
    ...
    while true do
        local ok, res = pcall(function()
            -- Server gets its expected 'true' argument here
...
function TDS:Place(TName, px, py, pz, ...)
    ...
    -- The script will no longer check for 'stack' or 'true' here,
    -- and will completely ignore the self:Addons() download loop.

    if GameState ~= "GAME" then
        return false 
    end

    -- Track existing towers owned by the local player
    local existing = {}
    ...
    -- Safely pass the extra arguments (like 'true') down to the placement function
    DoPlaceTower(TName, Vector3.new(px, py, pz), unpack(args))

    -- Wait for the new tower instance to appear in the workspace
    local NewT
    repeat
        for _, child in ipairs(workspace.Towers:GetChildren()) do
